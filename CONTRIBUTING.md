...
- The CSP (`src/middleware.ts`) is nonce-based — no `'unsafe-inline'` on `script-src`/`style-src`. This requires a fresh nonce per request embedded in the actual HTML, so `src/app/layout.tsx` sets `export const dynamic = 'force-dynamic'`, opting every page out of static prerendering. That includes the public `/creatorname` tipping page, this app's highest-traffic route — a deliberate trade given that page exists to request a wallet-signing action.
