...
function tipRequest(body: unknown, cookieValue?: string): NextRequest {
  requestCounter++;
  const headers = new Headers({
    'content-type': 'application/json',
    // Unique per request so the shared in-memory rate limiter doesn't let
    // one test's requests count against another's.
    'x-forwarded-for': `10.0.0.${requestCounter}`,
  });
  if (cookieValue) headers.set('cookie', `${WALLET_SESSION_COOKIE}=${cookieValue}`);
  return new NextRequest('http://localhost/api/tip', {
    method: 'POST',
    headers,
...
describe('POST /api/tip', () => {
  ...
  it("401s when sourcePublicKey does not match the caller's connected wallet", async () => {
    ...
    const response = await POST(
      tipRequest(
        { sourcePublicKey: SOURCE, destinationPublicKey: DEST, asset: 'XLM', amount: '5' },
        cookie,
      ),
    );
    expect(response.status).toBe(401);
